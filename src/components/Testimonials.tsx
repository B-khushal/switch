    ...
    <section className="py-32 bg-transparent">
      <div className="max-w-7xl mx-auto px-6 lg:px-8">
        ...
        <div className="text-center mb-20">
          <span className="inline-flex items-center px-3 py-1 rounded-full bg-[#FFF8EE] border border-brand/20 text-brand text-xs font-bold uppercase tracking-widest mb-4">Client Success</span>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {testimonials.map((item, index) => (
            <motion.div
              ...
              className="bg-white p-10 rounded-[32px] border border-black/5 shadow-2xl shadow-black/5 relative group"
            >
              <Quote className="absolute top-10 right-10 text-black/5 w-16 h-16 transform -scale-x-100" />
              
              <div className="flex gap-1 mb-8">
                {[1, 2, 3, 4, 5].map(star => (
                  <Star key={star} size={20} className="fill-brand text-brand" />
                ))}
              </div>
              
              <p className="text-lg text-[#111111] font-medium mb-10 relative z-10 leading-relaxed">
                "{item.quote}"
              </p>
              
              <div className="flex items-center gap-4">
                <img src={item.image} alt={item.author} className="w-14 h-14 rounded-2xl object-cover" />
                <div>
                  <div className="font-bold text-[#111111]">{item.author}</div>
